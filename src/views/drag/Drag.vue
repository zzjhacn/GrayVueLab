<template lang="html">
  <div class="hbox animated fadeIn m-n">
    <div class="col w">
      <div class="vbox">
        <div class="row-row">
          <ul class="list-group">
            <li class="list-group-item info">元件列表</li>
            <draggable v-model="comps" class="dragArea" :options="{group:{ name:'comp',  pull:'clone', put:false }}">
              <transition-group>
                <li class="list-group-item" v-for="(v, idx) in comps" :key="'comp' + idx">{{v.name}}</li>
              </transition-group>
            </draggable>
          </ul>
        </div>
      </div>
    </div>
    <div class="col">
      <div class="row m-n h-full">
        <div class="col-md-4 h-full">
          <div class="b center-block" style="width:337px">
            <table cellspacing="0" cellpadding="0" class="ode-SimpleMockFormPhoneBar" style="width: 100%; height: 24px;">
              <tbody>
                <tr>
                  <td align="right" width="" height="" rowspan="1" style="vertical-align: top;">
                    <img width="150" height="24" src="data:image/png;base64," ></img>
                  </td>
                </tr>
              </tbody>
            </table>
            <div class="ode-SimpleMockFormTitleBar" style="position: relative; overflow: hidden; width: 100%; height: 24px;">
              <div class="ode-SimpleMockFormTitle" style="text-align: left; position: absolute; left: 12px; top: 4px;">Screen1</div>
            </div>
            <div style="overflow: auto; position: relative; zoom: 1; width: 337px; height: 422px;">
              <div style="position: relative; zoom: 1;" ref="appPage">
                <div style="position: relative; overflow: hidden; background-color: rgb(255, 255, 255); background-image: url(&quot;&quot;); background-repeat: no-repeat; background-position: center center; background-size: 100% 100%; width: 320px; height: 422px;">
                  <draggable v-model="items" class="dragArea h-full" :options="{group:'comp'}">
                    <component v-bind:is="r.comp" :options="r.opt" v-for="(r, idx) in items" :key="this._uid" ref="items" @click="changeCurr(idx)"></component>
                  </draggable>
                </div>
              </div>
            </div>
            <table cellspacing="0" cellpadding="0" class="ode-SimpleMockFormPhoneBar" style="width: 100%; height: 35px;">
              <tbody>
                <tr>
                  <td align="center" style="vertical-align: top;">
                    <img width="239" height="35" src="data:image/png;base64,"></img>
                  </td>
                </tr>
              </tbody>
            </table>
          </div>
        </div>
        <div class="col-md-4">
          <div class="card card-accent-info w-full h-full">
            <div class="card-header">
              props of ({{currItem.name}})
            </div>
            <div class="card-body">
              <ul class="list-group">
                <li class="list-group-item" v-for="(v, k, idx) in props" :key="this._uid">
                  <div class="input-group">
                    <span class="input-group-addon">{{k}}</span>
                    <label v-if="v.type && v.type.name === 'Boolean'" :class="'switch switch-text switch-pill switch-primary float-right'">
                      <input type="checkbox" class="switch-input" v-model="currItem[k]">
                      <span class="switch-label" data-on="True" data-off="False"></span>
                      <span class="switch-handle"></span>
                    </label>
                    <v-select v-else-if="v.type && v.type.name === 'String' && v.opts" :options="v.opts" v-model="currItem[k]"></v-select>
                    <input v-else type="text" class="form-control" v-model="currItem[k]">
                    <!-- <span class="input-group-addon">{{v['type'] ? v['type'].name : 'String'}}</span> -->
                  </div>
                  <!-- <span class="help-block">{{v}}</span> -->
                </li>
              </ul>
            </div>
          </div>
        </div>
        <div class="col-md-4">
          <draggable v-model="items" class="dragArea h-full" :options="{group:'comp'}">
            <li class="list-group-item" v-for="(r, idx) in items" :key="this._uid" @click="changeCurr(idx)">{{r.name}}</li>
          </draggable>
        </div>
      </div>
    </div>
    <!-- <div class="col w">
      <div class="vbox">
        <div class="row-row">
          <div class="cell">
            <ul class="list-group">
              <li class="list-group-item" v-for="v in btns">{{v.l}}</li>
            </ul>
          </div>
        </div>
      </div>
    </div> -->
  </div>
</template>

<script>
import Draggable from 'vuedraggable'
import { buttonGroup, dropdown, radio, aside as sidebar, accordion, alert, datepicker, select as vSelect, option as vOption } from 'vue-strap'
import hub from '@/utils/eventhub'
import _ from 'lodash'
import CompDropdown from '@/comps/CompDropdown'
import Vue from 'vue'
export default {
  name: 'Drag',
  data () {
    return {
      //组件集合
      comps: [
        {name: 'Accordion', comp: accordion},
        {name: 'Alert', comp: alert},
        {name: 'Datepicker', comp: datepicker},
        {name: 'CompDropdown', comp: CompDropdown},
        {name: 'Dropdown', comp: dropdown}
      ],
      // 元素集合(拖拽产物)
      items: [],
      // 当前编辑节点
      currItem: '',
      p: {size: 'lg', text: 'testsss'},
      meta: 'dropdown'
    }
  },
  computed: {
    props () {
      if (!this.currItem) {
        return []
      }
      return this.currItem.props()
    }
  },
  methods: {
    changeCurr (idx) {
      this.currItem = this.$refs.items[idx]
    }
  },
  watch: {
    items (nv) {
      // console.log(nv)
      // let page = this.$refs.items
      // console.log(page)
      // console.log(page.children.size())
      _.forEach(nv, c => {
        console.log(c.comp.data)
        if(typeof(c) === 'VueComponent'){
        }
      })
      // hub.$emit('edit.curr_page', this.$refs.appPage)
    },
    currItem (nv, ov) {
      console.log(nv)
      nv && nv.$el.classList.toggle('curr')
      ov && ov.$el.classList.toggle('curr')
    }
  },
  components: {
    radio,
    dropdown,
    buttonGroup,
    Draggable,
    sidebar, accordion, alert, datepicker,CompDropdown, vSelect, vOption,
  },
  created () {
    _.forEach(this.comps, (v) => { if(!v.comp.name) {v.comp.name = v.name} } )
  },
  mounted () {
    hub.$emit('aside')
  },
  beforeDestroy () {
    hub.$emit('aside')
  },
  updated () {
    // hub.$emit('edit.curr_page', this.$refs.appPage)
  }
}

</script>

<style lang="css">
.curr {
  border: 2px solid;
  border-color: red;
}

.dragArea {
  min-height: 100px;
  background-color: #ccc;
}

.center-block {
  display: block;
  margin-right: auto;
  margin-left: auto;
}

.ode-SimpleMockFormPhoneBar {
  background-color: #000000;
}

.ode-SimpleMockFormTitleBar {
  background-color: dimgray;
  border-bottom: 1px solid lightgray;
}

.ode-SimpleMockFormTitle {
  color: white;
  font-size: 90%;
}

</style>
