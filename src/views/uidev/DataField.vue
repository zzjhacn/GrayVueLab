...
<div class="data-field">
  <div class="self" @click="toggle" :style="{ marginLeft: depth * 14 + 'px' }">
    ...
    <input type="text" :value="field.value" @blur="wb" @keydown.enter="wb">
  </div>
  <div class="children" v-if="expanded && isExpandableType">
    <data-field v-for="subField in limitedSubFields" :key="subField.key" :field="subField" :depth="depth + 1"></data-field>
    <span class="more" v-if="formattedSubFields.length > limit" @click="limit += 10" :style="{ marginLeft: (depth + 1) * 14 + 10 + 'px' }">...</span>
  </div>
</div>
</template>

<script>
import { UNDEFINED, INFINITY, NAN, isPlainObject, sortByKey } from './util'
import { modi } from './InstanceScanner'
