...
<div class="instance" :class="{
  ...
  <div class="self" @click.stop="select" @mouseenter="enter" @mouseleave="leave" :class="{ selected: selected }" :style="{ paddingLeft: depth * 15 + 'px' }">
    <span class="content">
  ...
  <div v-if="expanded">
    <instance v-for="child in sortedChildren" :key="child.id" :instance="child" :depth="depth + 1"></instance>
  </div>
</div>
</template>
...
import { selectInstance, highlighter } from './InstanceScanner'
...
export default {
  ...
  methods: {
    ...
    select() {
      selectInstance(this.instance.id)
      // bridge.send('select-instance', this.instance.id)
    },
    enter() {
      highlighter(this.instance.id)
