...
// import { highlight, unHighlight, getInstanceRect } from './meta/highlighter'
import { highlight, getInstanceRect } from './highlighter'
import { classify, camelize } from './util'
// import { stringify, classify, camelize } from './meta/util'
...
export function highlighter(id) {
  highlight(instanceMap.get(id))
};

export function modi(f, v) {
  const instance = instanceMap.get(currentInspectedId)
  console.log(v)
  console.log(f)
  console.log(instance)
  Vue.set(instance, f.key, v)
  flush()
}
/**
...
function flush() {
  ...
  store.commit('components/FLUSH', payload)
}
...
export function selectInstance(id) {
  ...
  flush()
  store.commit('components/RECEIVE_INSTANCE_DETAILS', getInstanceDetails(id))
}
